...
from holoviews.streams import Pipe, RangeXY, PlotSize, Buffer, DataStream, DataFrameStream
...
if get_dark_pnCCD:
    dark_pnCCD = np.load('dark_pnCCD.npy')
    ...
    for data in ds:
        ...
        if data['pnCCD_in_data']: ## skip analysis if now pn ccd in data
            ...
            if pnCCD_in_stream:
                ...
                if height_tof>250 or integral_tof>1e6:
                    hit_found = True
...
bokeh_buffer_last_hit_trainids = Div(text='<p><span style="font-size:20pt">#############<br><br>#############<br><br>#############<br><br>#############<br><br>#############<br><br><span></p>', width = 400, height = 400)
#~ print("...3")
...
bokeh_row_2 = row(bokeh_hits_pnCCD_list[1],bokeh_hits_pnCCD_list[2],bokeh_hits_pnCCD_list[3],bokeh_hits_pnCCD_list[4],bokeh_hits_pnCCD_list[0], bokeh_buffer_last_hit_trainids)
